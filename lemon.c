...
int main(int argc, char **argv)
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FLAG, "E", (char*)&useEnum, "use enum instead of #define."},
    {OPT_FLAG,0,0,0}
  };
  int i;
  int exitcode;
  ...
  if( rpflag ){
    ...
    ** generate the file for us.) */
    if( !mhflag ) ReportHeader(&lem, useEnum);
  }
  if( statistics ){
...
void ReportHeader(struct lemon *lemp, int useEnum)
{
  FILE *out, *in;
  const char *prefix;
  char line[LINESIZE];
  ...
  if( in ){
    ...
    if( fgets(line,LINESIZE,in) ){
      ...
      if( strcmp(line,pattern) ) comp = 0;
    }
    if( useEnum && comp == -1 ) {
	    if( useEnum && comp == -1 && fgets(line,LINESIZE,in) ){
	      lemon_sprintf(pattern,"typedef enum %s_token_e {\n", lemp->name);
	      if( strcmp(line,pattern) ) comp = 0;
	    }
	    for(i=1; comp == -1 && i<lemp->nterminal && fgets(line,LINESIZE,in); i++){
	      lemon_sprintf(pattern,"\t%s%-30s = %2d,\n",prefix,lemp->symbols[i]->name,i);
	      if( strcmp(line,pattern) ){ comp = 0; }
	    }
	    if( i==lemp->nterminal && fgets(line,LINESIZE,in) ){
	      lemon_sprintf(pattern,"} %s_token_e;\n", lemp->name);
	      if( strcmp(line,pattern) ) comp = 0;
	    }
    } else if( !useEnum && comp == -1 ){
	    for(i=1; comp == -1 && i<lemp->nterminal && fgets(line,LINESIZE,in); i++){
    ...
    nextChar = fgetc(in);
    fclose(in);
    if( i==lemp->nterminal && nextChar==EOF && comp == -1){
